...
from unittest.mock import Mock, call, create_autospec, patch, sentinel
...
class WatchdogTestCase(TestCase):
    ...
    def test_send(self) -> None:
        mocked_socket = create_autospec(socket.socket)
        cut = WatchDog(sock=mocked_socket, addr=self.TEST_ADDR)
        cut._send("Hello, world!")
        cut._socket.sendto.assert_called_once_with(b"Hello, world!", self.TEST_ADDR)  # type: ignore[attr-defined]
    ...
    def test_addr_parsing(self) -> None:
        # Standard address
    ...
    def test_notify_error_with_message(self) -> None:
        cut = WatchDog(sock=Mock(spec=["sendto"]), addr=self.TEST_ADDR)
        with patch("systemd_watchdog.WatchDog._send") as patched_send:
